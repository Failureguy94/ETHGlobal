...
export default function Home() {
  ...
  const addSepoliaNetwork = async () => {
    try {
      ...
      toast.error(`Failed to add Sepolia network: ${error.message}`);
    }
  };
    ...
    try {
      if (window.ethereum) {
        ...
        try {
          ...
          // If the network doesn't exist, add it (for Sepolia)
          if (switchError.code === 4902 && chainId === SEPOLIA_CHAIN_ID) {
        ...
        const networkName = chainId === SEPOLIA_CHAIN_ID ? "Sepolia" : "World";
        ...
        setNetworkName(chainId === SEPOLIA_CHAIN_ID ? "Sepolia" : "World");
  ...
  const fetchHealthFactor = async () => {
    ...
    try {
      ...
      if (currentChainId !== WORLD_CHAIN_ID) {
        const switched = await switchNetwork(WORLD_CHAIN_ID);
        if (!switched) return;
      }
  ...
  const deployCallbackContract = async () => {
    ...
    try {
      ...
      // Use Sepolia system contract as the callback sender
      const callbackSenderAddress = SEPOLIA_SYSTEM_CONTRACT;
      ...
      toast.success(
        `Callback contract deployed at: ${callbackAddress} on World`
      );
  ...
  // Deploy the reactive contract (on Sepolia )
  const deployReactiveContract = async () => {
    ...
    try {
      ...
      if (currentChainId !== SEPOLIA_CHAIN_ID) {
        toast.info(
          "Switching to Sepolia network for reactive contract deployment..."
        );
      ...
      toast.success(
        `Reactive contract deployed at: ${reactiveAddress} on Sepolia`
      );

      // Remind user about the block explorer
      toast.info(
        `View your reactive contract on Sepolia block explorer: https://sepolia.etherscan.io/address/${reactiveAddress}`
      );
    } catch (error: any) {
  ...
  const approveCollateralToken = async () => {
    ...
    setLoading(true);
    try {
      // Switch to WORLD for token approvals
      if (currentChainId !== WORLD_CHAIN_ID) {
        ...
        try {
          ...
          console.error("Network switching error:", switchError);
          toast.error(
            "Failed to switch networks. Please try switching to World network manually and try again."
          );
          setLoading(false);
          return;
        }
      ...
      const network = await provider!.getNetwork();
      if (network.chainId !== BigInt(WORLD_CHAIN_ID)) {
        toast.error(
          "Network switch did not complete successfully. Please try again."
        );
        setLoading(false);
      ...
      toast.success(
        `Successfully approved ${symbol} for the protection contract on World`
      );
    } catch (error: any) {
      console.error("Error approving sepolia:", error);
  ...
  const checkAllowance = async () => {
    ...
    setLoading(true);
    try {
      // Switch to World for checking allowance
      if (currentChainId !== WORLD_CHAIN_ID) {
        toast.info("Switching to World network to check allowance...");
      ...
      // Verify we're on World before proceeding
      const network = await provider!.getNetwork();
      if (network.chainId !== BigInt(WORLD_CHAIN_ID)) {
        toast.error(
          "Network switch did not complete successfully. Please try again."
  ...
  const calculateProtection = async () => {
    ...
    try {
      ...
      if (currentChainId !== WORLD_CHAIN_ID) {
        toast.info("Switching to World network to calculate protection...");
  ...
  useEffect(() => {
    if (window.ethereum) {
      const handleChainChanged = (chainId: string) => {
        ...
        } else if (chainIdNumber === WORLD_CHAIN_ID) {
          setNetworkName("World");
        } else {
              ...
              {!isConnected ? (
                ...
                <div className="text-center">
                  ...
                  <div className="mt-2 flex justify-center space-x-3">
                    <button
                      onClick={() => switchNetwork(WORLD_CHAIN_ID)}
              ...
              <div className="mt-10 space-y-8">
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                        Collateral Price Feed (Chainlink on World)
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                        Uniswap Pair to Monitor (Sepolia)
                      </label>
                      <input
                        type="text"
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="bg-blue-50 p-3 rounded-md mb-4">
                      ...
                      >
                        {loading
                          ? "Deploying..."
                          : "1. Deploy Callback Contract on World"}
                      </button>
                    ...
                    <div className="bg-purple-50 p-3 rounded-md">
                      ...
                      >
                        {loading
                          ? "Deploying..."
                          : "2. Deploy Reactive Contract on Sepolia"}
                      </button>
                      ...
                      {deployedReactiveAddress && (
                        <div className="text-sm text-gray-600 mt-2">
                          ...
                          <a
                            href={`https://sepolia.etherscan.io/address/${deployedReactiveAddress}`}
                            target="_blank"
                            rel="noopener noreferrer"
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h2 className="text-xl font-semibold text-gray-800">
                    3. Approve Collateral on World
                  </h2>

                  <div className="mt-4 space-y-4">
                    <div className="bg-blue-50 p-3 rounded-md">
                      <p className="text-xs text-blue-800 mb-2">
                        This step will automatically switch to World network
                      </p>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                ...
                <div className="bg-gray-50 p-6 rounded-lg">
                  <h2 className="text-xl font-semibold text-gray-800">
                    4. Check Protection Status on World
                  </h2>
